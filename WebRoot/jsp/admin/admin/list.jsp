...
<body>
	<jsp:include page="../common/header.jsp">
		<jsp:param value="admin" name="fun" />
	</jsp:include>
	...
	<script>
		layui.use([ 'layer', 'table', 'element' ], function() {
			...
			var tableIns = table.render({
				elem : '#TABLE_ADMIN' //指定原始表格元素选择器（推荐id选择器）
				,
				id : 'adminTable',
				cols : [ [ {
					field : 'id',
			...
			//监听工具条
			table.on('tool(admin)', function(obj) { //注：tool是工具条事件名，adminTable是table原始容器的属性 lay-filter="对应的值"
				var data = obj.data; //获得当前行数据
				var layEvent = obj.event; //获得 lay-event 对应的值
				var id = data.id;
				if (layEvent === 'del') { //删除
					layer.confirm('确定删除这个管理员么？', function(index) {
		...
		function doAddPage(id) {
			...
			layer.open({
				...
				shade : 0.8,
				area : [ '30%', '450px' ],
				content : '${ctx}/admin/admin/editPage'
			});
		}
		...
		function doEditPage(id) {
			...
			layer.open({
				...
				shade : 0.8,
				area : [ '30%', '450px' ],
				content : '${ctx}/admin/admin/editPage?id=' + id
			});
		}

		// 删除
		function doDelete(id) {
			var url = '${ctx}/admin/admin/delete';
			var data = {
				id : id
			}
